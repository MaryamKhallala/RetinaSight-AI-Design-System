  ...
  const api = {
    ...
    async uploadImage(fileBlob, opts) {
      ...
      if (!API) {
        ...
        return api.addCase({ laterality: opts.laterality, device: opts.device });
      }
      ...
      const token = (function () { try { return localStorage.getItem('octopus.token'); } catch (e) { return null; } })();
    ...
    async _hashPwd(pwd) {
      ...
      } catch (e) { return 'plain:' + pwd; }   // repli si crypto indisponible
    },
    _localUsers() {
      try { return JSON.parse(localStorage.getItem('octopus.accounts') || '{}'); } catch (e) { return {}; }
    },
    _saveLocalUsers(map) {
      try { localStorage.setItem('octopus.accounts', JSON.stringify(map)); } catch (e) {}
    },
    _setSession(session) {
      try { localStorage.setItem('octopus.session', JSON.stringify(session)); } catch (e) {}
      audit('auth.login', { email: session.email, role: session.role });
      commit();
      return session;
    },

    async register({ email, password, role, name, dept }) {
      ...
      if (API) {
        try {
          const r = await fetch(API + '/auth/register', {
            ...
            body: JSON.stringify({ email, password, role, name, dept }),
          });
          const d = await r.json().catch(() => ({}));
          if (!r.ok) return { ok: false, error: d.detail || 'Inscription impossible.' };
          if (d.token) { try { localStorage.setItem('octopus.token', d.token); } catch (e) {} }
          return { ok: true, session: this._setSession({ email: d.user.email, role: d.user.role, name: d.user.name, at: new Date().toISOString() }) };
        } catch (e) { /* repli local si réseau coupé */ }
      }
      // mode local
      const users = this._localUsers();
      if (users[email]) return { ok: false, error: 'Un compte existe déjà avec cet e-mail. Connectez-vous.' };
      users[email] = { email, role, name, dept: dept || '—', hash: await this._hashPwd(password), at: new Date().toISOString() };
      this._saveLocalUsers(users);
      audit('auth.register', { email, role });
      return { ok: true, session: this._setSession({ email, role, name, at: new Date().toISOString() }) };
    },
    ...
    async login({ email, password, role }) {
      ...
      if (API) {
        try {
          ...
          if (r.status === 404) return { ok: false, error: 'NO_ACCOUNT' };   // → l'UI propose l'inscription
          return { ok: false, error: d.detail || 'E-mail ou mot de passe incorrect.' };
        } catch (e) { /* repli local */ }
      }
      // mode local
      const users = this._localUsers();
      ...
      if (u.hash !== await this._hashPwd(password)) return { ok: false, error: 'E-mail ou mot de passe incorrect.' };
      return { ok: true, session: this._setSession({ email: u.email, role: role || u.role, name: u.name, at: new Date().toISOString() }) };
    },

    session() {
      try { return JSON.parse(localStorage.getItem('octopus.session') || 'null'); } catch (e) { return null; }
    },
    logout() {
      try { localStorage.removeItem('octopus.session'); localStorage.removeItem('octopus.token'); } catch (e) {}
      audit('auth.logout', {});
      commit();
